...
function updateSelectedModel() {
  ...
  if (changeModelLinkEl) {
    changeModelLinkEl.href = selectedType ? `models.html?type=${encodeURIComponent(selectedType)}` : "models.html";
  }
}
...
function updatePrices() {
  ...
  priceValues.forEach((priceEl) => {
    priceEl.textContent = isMonthly ? priceEl.dataset.priceMonthly : priceEl.dataset.priceYearly;
  });
  ...
  planSubnotes.forEach((noteEl) => {
    noteEl.textContent = isMonthly ? "رسوم تخصيص أولية" : "التخصيص مجاني";
  });

  planSavings.forEach((savingEl) => {
    savingEl.textContent = isMonthly ? "" : savingEl.dataset.originalText || savingEl.textContent;
...
planLinks.forEach((linkEl) => {
  linkEl.addEventListener("click", (event) => {
    if (!selectedModel) {
      ...
      window.location.href = selectedType ? `models.html?type=${encodeURIComponent(selectedType)}` : "models.html";
    }
  });
});
